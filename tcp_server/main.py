...
class TcpServer:
    user_name = {}  # 存储客户端信息； dict 用户名：小飞机名字序号
    ...
    def start(self):
        ...
        while True:
            try:
                conn, addr = self.tcp_socket.accept()       # 监听小飞机和env的地址和发送的消息
    ...
    def do_request(self, conn):
        ...
        conn_node = Node()
        while True:
            recv_data = conn.recv(1024).decode('utf-8').strip()     # 获取小飞机和env发来的数据
            info_list = recv_data.split(" ")        # 切割命令

            # 如果接收到命令为exit，则表示小飞机不玩了，回家了，拜拜，关闭连接
            if recv_data == "exit":
                msg = self.get_time() + " 小飞机" + conn_node.Name + "不玩了回家了！"
                print(msg)
                ...
                try:
                    ...
                    conn.send((self.get_time() + ' 小飞机跑丢了，请重试！').encode('gb2312'))
                    continue

            if info_list[-1] == '-n':
                # 新用户注册
                print(self.get_time() + ' 小飞机：' + info_list[0] + '连接成功！')
                data_info = self.get_time() + ' 欢迎小飞机：' + info_list[0] + '加入组织！'
